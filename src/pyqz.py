...
def get_grid(ratios, coeffs=[[1,0],[0,1]],
    ...
    fn = pyqz_tools.get_MVphotogrid_fn(Pk = Pk, kappa = kappa, struct = struct, 
                                       sampling = sampling)
    metadata = pyqz_tools.get_MVphotogrid_metadata(fn)
            ...
            for s in range(n_seg):
                ...
                for seg_b in grid_segs:
                    ...
                    if not(seg_a[0] in seg_b) and not(seg_a[1] in seg_b):
                        ...
                        if pyqz_tools.seg_intersect(seg_a[0],seg_a[1],seg_b[0],seg_b[1]):
...
def get_global_qz_singlespec((j, final_cols, data, data_cols, which_grids,
    ...
    for (i,calc) in enumerate(final_cols):    
        ...
        if ratios in diagnostics and not('KDE' in calc) :
            qz = calc.split('|')[1]
            Rval = []
            ...
            coeffs = diagnostics[ratios]['coeffs']
    ...
    if KDE_pickle_loc:
        ...
        fn_out = pyqz_tools.get_KDE_picklefn(ids, Pk, kappa, struct, sampling, KDE_method)
        # Append the path:
...
def get_global_qz(data, data_cols, which_grids,
    ...
    if len(np.shape(data)) ==1:
        sys.exit('Bad data shape: "data" should be a 2-D array, but np.shape(data)=%s.' % 
                 np.shape(data))  
    ...
    if nproc == -1:
        ...
        sys.exit('nproc set to %i, but I can find only %i cpus.' % (nproc, ncpu_max))
    ...
    for j in range(len(jobs)):      
        ...
        if results[j][0] != j :
            sys.exit('Something went very badly wrong ... (L1203)')
...
def get_global_qz_ff(fn, 
    ...
    if suffix_out =='':
        sys.exit("suffix_out='' would overwrite the input file. Specify something else.")
